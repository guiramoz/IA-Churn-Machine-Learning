...
#criar a simulação da lógica de churn COM RUÍDO (mais realista)
#o cliente tem mais chance de sair se tiver muitas reclamações OU
#se o contrato for curto, mas com aleatoriedade (ruído)
cancelou_base = ((df['reclamacoes']>2)|(df['tempo_contrato']<6)).astype(float)
ruido = np.random.normal(0, 0.15, n_registros)  # Adiciona variação realista
cancelou_probabilidade = cancelou_base + ruido
df['cancelou'] = (cancelou_probabilidade > 0.5).astype(int)
